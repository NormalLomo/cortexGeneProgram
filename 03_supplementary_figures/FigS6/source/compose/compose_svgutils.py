...
def compose(cropdir, template, out_svg, tag_size, tag_dx, tag_dy, with_tags, title,
            tag_sequential=False, tag_start=0):
    pg = template["page"]
    ...
    # spacer panels (gutter rows) carry no content tag and do NOT consume a letter
    NO_TAG_IDS = {"spacer"}
    tag_idx = 0  # counts only tagged panels, so spacers don't shift the sequence
    for i, p in enumerate(template["panels"]):
        ...
        if with_tags and str(p["id"]) not in NO_TAG_IDS:
