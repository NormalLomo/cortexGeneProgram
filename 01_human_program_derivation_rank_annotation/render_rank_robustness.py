...
plt.rcParams.update({'pdf.fonttype': 42, 'ps.fonttype': 42, 'font.family': 'sans-serif', 'font.sans-serif': ['Nimbus Sans', 'Liberation Sans', 'DejaVu Sans'], 'font.size': 7, 'axes.titlesize': 8, 'axes.labelsize': 7, 'xtick.labelsize': 6.5, 'ytick.labelsize': 6.5, 'legend.fontsize': 6, 'axes.linewidth': 0.6, 'xtick.major.width': 0.6, 'ytick.major.width': 0.6, 'xtick.major.size': 2.5, 'ytick.major.size': 2.5, 'axes.spines.top': False, 'axes.spines.right': False, 'legend.frameon': False, 'figure.dpi': 200})
ALTK = [40, 50, 65, 70]
...
gs = fig.add_gridspec(2, 12, height_ratios=[1.0, 1.05], hspace=0.55, wspace=2.4, left=0.075, right=0.965, top=0.955, bottom=0.085)
axa = fig.add_subplot(gs[0, 0:5])
x = np.arange(len(ALTK))
w = 0.38
...
b2 = axa.bar(x + w / 2, sc, w, color=[KCOL[k] for k in ALTK], edgecolor='black', linewidth=0.5, alpha=0.55, hatch='///', label='conf. match (r>0.5)')
axa.axhline(0, color='0.4', lw=0.6)
axa.set_xticks(x)
axa.set_xticklabels([f'K{k}' for k in ALTK])
...
axc.set_xticks(xc)
axc.set_xticklabels(['K60*'] + [f'K{k}' for k in ALTK])
axc.set_ylabel('fraction of top-6 drivers')
axc.set_ylim(0, 1.0)
axc.set_title('b  Top cell-driver recurrence', loc='left', fontweight='bold')
...
axd.set_title('c  Most-variable program', loc='left', fontweight='bold', fontsize=7.0)
axd.axis('off')
ks = [60] + ALTK
nrow = len(ks)
axd.set_xlim(0, 1)
axd.set_ylim(0, nrow + 0.6)
axd.text(0.02, nrow + 0.15, 'K', fontsize=6.5, fontweight='bold')
axd.text(0.24, nrow + 0.15, 'η²', fontsize=6.5, fontweight='bold')
axd.text(0.45, nrow + 0.15, 'assoc. subclass', fontsize=6.0, fontweight='bold')
for (i, k) in enumerate(ks):
    row = tv.loc[tv.K == k].iloc[0]
    y = nrow - 1 - i + 0.5
    is_lam = bool(row['is_laminar_IT'])
    col = '#2E7D32' if is_lam else '#9E9E9E'
    axd.add_patch(Rectangle((0.0, y - 0.34), 1.0, 0.68, facecolor=col, alpha=0.16, edgecolor='none', zorder=0))
    ...
    mark = ' ✓' if is_lam else ''
    axd.text(0.45, y, sc_txt + mark, fontsize=5.2, va='center', color='#1B5E20' if is_lam else '0.25')
...
for (j, K) in enumerate(ALTK):
    ...
    sca = axb.scatter(s['eta_60'], s['eta_K'], c=cc, cmap='viridis', vmin=0.3, vmax=1.0, s=14, edgecolor='black', linewidth=0.25, zorder=3)
    ...
    axb.text(0.04, 0.93, f'ρ={rho:.2f}\nr={pear:.2f}', transform=axb.transAxes, fontsize=5.8, va='top', ha='left')
    axb.set_xlabel('η²  (K60)')
    if j == 0:
        axb.set_ylabel('η²  (alt-K, matched)')
    axb.tick_params(labelsize=5.8)
fig.text(0.075, 0.49, 'd  Per-program variability (η²): alt-K vs K60, gene-spectra matched', fontsize=8, fontweight='bold', ha='left')
cax = fig.add_axes([0.4, 0.018, 0.22, 0.013])
...
fig.savefig(pdf.replace('.pdf', '.svg'))
