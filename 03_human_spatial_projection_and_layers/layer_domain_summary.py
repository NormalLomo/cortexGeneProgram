...
for (bp, br) in zip(itp, itr):
    dp = bp.to_pandas().set_index('bin')
    dr = br.to_pandas().set_index('bin')
...
fdat = pd.concat(parts).rename(columns=OLD_TO_NEW)
...
WM_OLD = ['program_37', 'program_26', 'program_5', 'program_56']
WMPROGS = [OLD_TO_NEW[p] for p in WM_OLD]
...
for b in pf.iter_batches(batch_size=400000, columns=['bin', 'OLIGO', 'rctd_pass_mask']):
    d = b.to_pandas()
    d = d[d['bin'].isin(binset)]
    if len(d):
        rc.append(d)
rc = pd.concat(rc).set_index('bin')
pc = []
pf = pq.ParquetFile(base + 'spatial_bin50_program_score_SCT.parquet')
for b in pf.iter_batches(batch_size=400000, columns=['bin'] + WM_OLD):
    d = b.to_pandas()
    d = d[d['bin'].isin(binset)]
    if len(d):
        pc.append(d)
pc = pd.concat(pc).rename(columns=OLD_TO_NEW).set_index('bin')
...
for ch in CHIPS_H:
    ...
    seeds = pts[sub['OLIGO'].to_numpy() >= 0.5]
    if len(seeds) < 20:
