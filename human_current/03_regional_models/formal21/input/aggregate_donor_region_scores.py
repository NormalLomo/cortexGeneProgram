...
def main():
    ...
    obs = pd.read_csv(
        args.obs,
        usecols=lambda c: c in {"Unnamed: 0", "donor", "batch", "region"},
        index_col=0,
    )
    ...
    for chunk in reader:
        ...
        if meta[["donor", "batch", "region"]].isna().any(axis=None):
            missing = int(meta[["donor", "batch", "region"]].isna().any(axis=1).sum())
            raise ValueError(f"{missing} score rows lack donor, batch, or region metadata")
        chunk = chunk.rename(columns=rename)[programs].astype(float)
        joined = meta.join(chunk)
        grouped = joined.groupby(["donor", "batch", "region"], observed=True)
        c = grouped.size().astype(np.int64)
        s = grouped[programs].sum()
        ss = joined.assign(**{p: joined[p] ** 2 for p in programs}).groupby(
            ["donor", "batch", "region"], observed=True
        )[programs].sum()
    ...
    wide = means.copy()
    wide["n_cells"] = counts
    wide.reset_index().to_csv(outdir / "donor_region_means.tsv", sep="\t", index=False)
    ...
    for program in programs:
        ...
        part.insert(3, "program", program)
        long.append(part)
    long = pd.concat(long, ignore_index=True)
    long.to_csv(outdir / "donor_region_program_summary.tsv", sep="\t", index=False)
