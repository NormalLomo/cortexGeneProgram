...
#
# PRODUCES retained panel inputs into the explicit output directory:
...
#                    snRNA_1M.h5ad  (SCT-corrected, depth-equalized, rounded counts in X; not SCT data/log expression)
#
# NORMALIZATION:
#   marker expression = log1p( corrected_count / cell_total_corrected_counts * 1e4 )
#   i.e. scanpy normalize_total(target_sum=1e4) then log1p. Per-cell totals
#   use the cell's FULL corrected-count transcriptome (all 32,649 atlas genes).
#
# SPECIFICITY metric:
...
PROJ = (__import__("os").environ["CORTEX_PROGRAM_ROOT"] + "")
...
ATLAS = os.path.join(os.environ["SNRNA_WORK_DIR"], "snRNA_1M.h5ad")
...
if __name__ == "__main__":
    ...
    ap.add_argument("--outdir", required=True, help="Directory for retained panel inputs.")
    args = ap.parse_args()
    main(args.outdir)
