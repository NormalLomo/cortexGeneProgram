#!/usr/bin/env python3
# =====================================================================
# Datablock 5-panel INK-CROP (b,c,f,i,j only) — reuses the exact crop_one
# logic from scripts/fig1/fig1_ink_crop.py, but limited to the datablock
# subset so panels a,d,e,g,h are NOT touched. c/i were just re-rendered
# (new SLAT Dual-PCA UMAP coords) so MUST be re-cropped; b/f/j re-cropped
# for consistency (idempotent: bbox of an already-cropped SVG == itself).
# =====================================================================
import os, re, json, subprocess, tempfile
from lxml import etree

PROJ = "CORTEX_PROGRAM_ROOT"
SVGD = os.path.join(PROJ, "figures/fig1/svg_panels")
...
def crop_one(pid):
    svg_path = os.path.join(SVGD, f"fig1_{pid}.svg")
...
def main():
    ...
    side = os.path.join(SVGD, "_cropped_aspects_datablock5.json")
