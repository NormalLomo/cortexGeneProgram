...
# fig1 version hardcodes the fig1 dir + "abcdefghijklij" so this is a fig2
# sibling pointed at scripts/figmarkcorr_A/svg_panels and looping "abcdefghijkl").
# For each panel SVG (scripts/figmarkcorr_A/svg_panels/figA_{a..h}.svg):
...
SVGD = os.path.join(PROJ, "scripts/figmarkcorr_A/svg_panels")
...
def crop_one(pid):
    svg_path = os.path.join(SVGD, f"figA_{pid}.svg")
...
def main():
    ...
    for pid in list("abcdefghijkl"):
        r = crop_one(pid)
        published = os.path.join(SVGD, f"{pid}.svg")
        if not os.path.islink(published):
            shutil.copyfile(os.path.join(SVGD, f"figA_{pid}.svg"), published)
        results[pid] = r
