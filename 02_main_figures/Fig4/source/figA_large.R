...
# RENUMBER 20260623: program_names.tsv now has new_P ("P1"-"P54" + "EXCLUDED") and cnmf_component (old 1-60).
# pid = old component number; new_pid = display number (1-54); EXCLUDED rows = cohort-technical, dropped here.
nm    <- nm[new_P != "EXCLUDED"]  # keep 54 biological programs only
nm[, pid := cnmf_component]  # old program index (for data lookup, B column = "program_N")
nm[, new_pid := as.integer(sub("^P", "", new_P))]  # new display number P1-P54
nm[, weak := (confidence == "brain-weak")]  # star == 24 brain-weak whitelist; fdr>0.05 brain-sig NOT starred
nm[, plab := sprintf("P%d %s%s", new_pid, name_short, ifelse(weak, "*", ""))]  # display uses NEW pid
plab_map <- setNames(nm$plab, paste0("program_", nm$pid))  # key = old program index
pshort   <- setNames(nm$name_short, paste0("program_", nm$pid))
# brain-weak star (24-program whitelist == confidence brain-weak): propagate to pshort-based labels too
...
# Between-chip headline key set (for panel b forest plot filtering)
...
Mm <- Mm[, ord_cols]
col_pid <- as.integer(sub("program_", "", ord_cols))  # old program index
col_wk  <- ifelse(nm$weak[match(col_pid, nm$pid)], "*", "")  # brain-weak star on heatmap col labels
col_new_pid <- nm$new_pid[match(col_pid, nm$pid)]  # new display number P1-P54
...
# Force-include VIP×P30 (program_33) and L6 IT×P49 (program_54) as micro-niche exemplars
...
# ---------------------------------------------------------------------
# RASTERISE dense spatial scatter ONLY. The spatial panels below draw
# tens of thousands of shape=15 square markers per facet; emitting them
# as vector geom_point blows up the SVG (>150MB) and trips rsvg's 1e6
# XML-element cap during ink-crop. Override geom_point -> ggrastr
# rasterise (dpi=400, ragg) for ALL subsequent (spatial) layers ONLY.
# Abstract panels a-l above are already built with vector geom_point
# (defined before this override) and remain unaffected / fully vector.
...
# =====================================================================
# NEW AVOIDANCE 1 : P51 Reactive-astro/vascular  (-)  upper-layer IT
#   region-contrast, COMPACT to 2 rows (M1 strong | V1 absent), 2 cols each:
#   [P51 SCT field]  [upper-IT weight field + P51 high-zone overlay (blue)]
#   In M1 the P51 high-zone sits where IT is empty (exclusion); in V1 they overlap.
# =====================================================================
...
p8_short  <- avnm[["p8_short"]]       # "Neurofilament cytoskeleton (pan-neuronal)" (P8, from program_names.tsv)
av2_r     <- avnm[["avoid2_r"]]
...
# =====================================================================
# NEW AVOIDANCE 2 : OPC  (-)  P8 neurofilament cytoskeleton (pan-neuronal) — single M1 chip
#   [OPC weight]  [P8 SCT]  [OPC weight + P8 high-zone overlay (blue)]
...
a2_p8   <- av2_panel(av2, "p8_s",  NULL,      sprintf("%s (P8)\nSCT score", p8_short), "rel.\nSCT", viridis=TRUE)
...
# =====================================================================
# ASSEMBLE : portrait ~180 x 250 mm
# Order (rule: pan-region commonality -> spatial grounding -> region diff):
#   a (heatmap, full width)
#   b | c
#   d | e
#   S = HERO spatial grid (full width, prominent centerpiece)
#   Z = zoom (full width, short)
#   f | g
#   h (full width)
# =====================================================================
# add order: A=pa B=pb C=pc D=pd E=pe F=hero G=zoom H=avoid1 I=avoid2 J=pf K=pg L=ph
# F (hero spatial grid) = full-width prominent centerpiece.
# SPATIAL-AVOIDANCE BLOCK = G (L2-3 x myelin zoom) + H (P51 x upper-IT region contrast)
#   + I (OPC x P8 neuropil) read together, full-width bands.
# J/K/L = region-modulation block (was h/i/j).
design <- "
AAAA
AAAA
AAAA
...
LLLL
"
fig <- pa + pb + pc + pd + pe + hero_block + zoom_block +
       avoid1_block + avoid2_block + pf + pg + ph +
  plot_layout(design=design,
    heights=c(0.62,0.62,0.62,0.62,0.62,0.62,0.62, 0.92,0.92, 0.92,0.92,
              1.55,1.55,1.55, 1.00,1.00, 1.10,1.10,1.10,1.10, 1.05,1.05,1.05,
              0.92,0.92, 0.82,0.82)) +
  plot_annotation(tag_levels="a")

PAGE_H <- 300  # mm (grown from 258 to fit 2 added avoidance bands h-i; width unchanged 180)
pdf_path <- file.path(OUT, "figA_large.pdf")
png_path <- file.path(OUT, "figA_large.png")
cairo_pdf(pdf_path, width=180/25.4, height=PAGE_H/25.4, family=BASE_FONT)
print(fig); dev.off()

ragg_ok <- requireNamespace("ragg", quietly=TRUE)
if (ragg_ok) {
  ragg::agg_png(png_path, width=180/25.4, height=PAGE_H/25.4, units="in", res=200)
  print(fig); dev.off()
} else {
  png(png_path, width=180/25.4, height=PAGE_H/25.4, units="in", res=200, type="cairo")
  print(fig); dev.off()
}
# SVG (true vector text; rasterised spatial scatter embedded as bitmap, so
# no XML-element blow-up). Same fig object -> identical layout to the PDF.
svg_path <- file.path(OUT, "figA_large.svg")
if (requireNamespace("svglite", quietly=TRUE)) {
  svglite::svglite(svg_path, width=180/25.4, height=PAGE_H/25.4)
  print(fig); dev.off()
}
cat("DONE\n", pdf_path, "\n", png_path, "\n", svg_path, "\n")
