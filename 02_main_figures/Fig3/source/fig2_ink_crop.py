...
# sibling pointed at figures/fig2/svg_panels and looping "abcdefgh").
# For each panel SVG (figures/fig2/svg_panels/fig2_{a..h}.svg):
...
SVGD = os.path.join(PROJ, "figures/fig2/svg_panels")
...
def crop_one(pid):
    svg_path = os.path.join(SVGD, f"fig2_{pid}.svg")
...
def main():
    results = {}
    for pid in "abcdefgi":  # h dropped (merged into e) per user FB v6
        r = crop_one(pid)
        results[pid] = r
