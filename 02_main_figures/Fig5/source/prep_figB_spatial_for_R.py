...
P_MYELIN, P_OLIGODEV, P_PROTON, P_MICRO, P_SYN = 45, 13, 8, 40, 29  # P29 Glutamatergic synapse: gene-independent (gene_cos=0.21) valid synaptic hub partner of P40; per-bin r(V1)=0.793 (orig P52 cohort-technical excluded)
...
NAME = {int(p): str(s) for p, s in zip(pn['program'], pn['name_short'])}
def name(prog):
    return NAME.get(int(prog), f'P{prog}').rstrip('*').rstrip()
...
print('sct...')
progs = [P_MYELIN, P_OLIGODEV, P_PROTON, P_MICRO, P_SYN]
...
SP1 = [(P_MYELIN, f'{name(P_MYELIN)} (P{P_MYELIN})', 'viridis'),
       (P_OLIGODEV, f'{name(P_OLIGODEV)} (P{P_OLIGODEV})', 'viridis'),
       (P_PROTON, f'{name(P_PROTON)} (P{P_PROTON})', 'magma')]
for ci, chip in enumerate(CHIP_LIST):
    ...
    for pj, (prog, ttl, cmap) in enumerate(SP1):
        ...
        t['prog'] = prog; t['ptitle'] = ttl; t['cmap'] = cmap
        t['row'] = ci; t['col'] = pj
        rows.append(t)
...
rows2 = []
for prog, ttl, cmap in [(P_MICRO, f'{name(P_MICRO)} (P{P_MICRO})', 'viridis'),
                        (P_SYN, f'{name(P_SYN)} (P{P_SYN})', 'viridis')]:
