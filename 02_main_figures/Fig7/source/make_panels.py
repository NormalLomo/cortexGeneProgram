#!/usr/bin/env python3
"""Render the Fig. 7 laminar and spatial panels used by the composer."""
import argparse
import os
import sys
import warnings
...
BASE = str(canonical_root / "results/xspecies_humanmap_v1/spatial_xspecies")
AGG  = f"{BASE}/_aggregate"
HUMAN_DIR = str(canonical_root / "results/crossregion_v1/_spatial_score_perchip")
OUT  = f"{BASE}/figures/Fig_spatial_univ/panels"
...
# Panel b uses a 1x4 vertical stack for the narrow right column.
def panel_b():
    ...
    # Map source component columns to retained display IDs:
    ...
    for ax, (Pid, prog, kind) in zip(axes, progs):
        ...
        # The mouse L1 annotation is documented in the figure legend.
...
# Panel e uses per-field z-scoring in a compact bottom band.
def panel_e_spatial():
    ...
    FIELD = LinearSegmentedColormap.from_list("field", ["#2a5d74", "#7fb2c0", "#eef0ee",
                                                        "#f0b27a", "#c0492f"])
    # One row x six columns forms the full-width bottom band.
...
BTWNDIR = str(canonical_root / "results/crossregion_v1/markcorr_betweenchip_v1")
RENUM_PATH = str(canonical_root / "results/crossregion_v1/program_renumber_map.tsv")
...
if __name__ == "__main__":
    ...
    panel_e_spatial()
    panel_aggregate()
    print("PANELS (b 1x4 stack, e compact, plus aggregate panel) DONE")
