...
D = str(PROJECT_ROOT / 'results/crossregion_v1')
OUT = os.environ.get('CORTEX_FIGURE_OUTPUT', f'{D}/markcorr_v2/spatial_for_R')
...
name_id = 'new_P' if 'new_P' in pn.columns else 'program'
name_ids = pd.to_numeric(pn[name_id].astype(str).str.removeprefix('P'), errors='coerce')
NAME = {int(p): str(s) for (p, s) in zip(name_ids.dropna(), pn.loc[name_ids.notna(), 'name_short'])}
...
pd.DataFrame([{'source_old_id': source_id, 'retained_id': retained_id(source_id), 'source_column': f'program_{source_id}', 'display_name': name(source_id)} for source_id in SOURCE_PROGRAMS]).to_csv(f'{OUT}/program_id_provenance.tsv', sep='\t', index=False)
meta = pq.read_table(f'{D}/spatial_bin50_meta.parquet', columns=['bin', 'chip', 'x', 'y', 'region']).to_pandas()
meta = meta[meta.chip.isin(CHIP_LIST)].reset_index(drop=True)
keep = set(meta['bin'])
rctd = pq.read_table(f'{D}/spatial_bin50_rctd_weights.parquet', columns=['bin', 'rctd_pass_mask']).to_pandas()
rctd = rctd[rctd['bin'].isin(keep)].reset_index(drop=True)
progs = list(SOURCE_PROGRAMS)
cols = [f'program_{i}' for i in progs]
sct = pq.read_table(f'{D}/spatial_bin50_program_score_SCT.parquet', columns=['bin', 'bin_total_umi'] + cols).to_pandas()
...
def panel_table(sub, prog):
    col = f'program_{prog}'
    out = pd.DataFrame({'x': sub['x'].values, 'y': sub['y'].values, 'val': sub[col].values.astype(float), 'valid': sub['valid'].values})
    vv = out.loc[out['valid'] & np.isfinite(out['val']), 'val'].values
    if len(vv):
        vmax = np.percentile(vv, 99)
        lo = np.percentile(vv, 1)
...
rows = []
SP1 = [(P_MYELIN, f'{name(P_MYELIN)} (P{retained_id(P_MYELIN)})', 'viridis'), (P_OLIGODEV, f'{name(P_OLIGODEV)} (P{retained_id(P_OLIGODEV)})', 'viridis'), (P_PROTON, f'{name(P_PROTON)} (P{retained_id(P_PROTON)})', 'magma')]
for (ci, chip) in enumerate(CHIP_LIST):
    sub = df[df.chip == chip]
    for (pj, (prog, ttl, cmap)) in enumerate(SP1):
        (t, vmin, vmax) = panel_table(sub, prog)
        t['chip'] = chip
        t['region_lab'] = CHIPS[chip]
        t['prog'] = retained_id(prog)
        t['ptitle'] = ttl
        t['cmap'] = cmap
        t['row'] = ci
        t['col'] = pj
...
sp1[['x', 'y', 'val', 'valid', 'vmin', 'vmax', 'chip', 'region_lab', 'prog', 'ptitle', 'cmap', 'row', 'col']].to_csv(f'{OUT}/sp1_long.csv', index=False)
mchip = 'D00865B3'
subm = df[df.chip == mchip]
rows2 = []
for (prog, ttl, cmap) in [(P_MICRO, f'{name(P_MICRO)} (P{retained_id(P_MICRO)})', 'viridis'), (P_SYN, f'{name(P_SYN)} (P{retained_id(P_SYN)})', 'viridis')]:
...
sp2[['x', 'y', 'val', 'valid', 'vmin', 'vmax', 'prog', 'ptitle', 'cmap']].to_csv(f'{OUT}/sp2_maps.csv', index=False)
v = subm['valid'].values
