...
parser.add_argument('--block-cells', type=int, default=10000)
...
usecols = [bc_col, 'derived_cell_libs', 'library', 'region', 'sub_region', 'brain_struct', 'donor_id', 'sex', 'Annotation', 'cell_class', 'num_cells_postQC']
...
d_data = X.create_dataset('data', shape=(0,), maxshape=(None,), dtype='uint32', chunks=(2 ** 20,), compression='gzip', compression_opts=2)
d_ind = X.create_dataset('indices', shape=(0,), maxshape=(None,), dtype='int32', chunks=(2 ** 20,), compression='gzip', compression_opts=2)
...
obs_g.attrs['column-order'] = np.array([], dtype='S1')
...
var_g.attrs['column-order'] = np.array([], dtype='S1')
var_g.create_dataset('_index', data=np.array([str(g).encode() for g in var_idx]))
Xraw = f['X']
BLK = args.block_cells
sel_mask = np.zeros(n_cells, dtype=bool)
sel_mask[rows_sorted] = True
nnz_total = 0
ptr_offset = 0
written_cells = 0
for start in range(0, n_cells, BLK):
    ...
    if local.size == 0:
        continue
    sub = Xraw[start:end, :][local, :]
    m = sp.csr_matrix(sub)
    bn = m.data.shape[0]
    d_data.resize((nnz_total + bn,))
    d_data[nnz_total:nnz_total + bn] = m.data.astype('uint32')
    d_ind.resize((nnz_total + bn,))
    d_ind[nnz_total:nnz_total + bn] = m.indices.astype('int32')
    nrows_blk = m.shape[0]
...
with open(os.path.join(OUT, 'barcodes.txt'), 'w') as fh:
    fh.write('\n'.join(bc_sorted) + '\n')
pd.DataFrame({'gene': var_idx, 'gene_name': gene_name}).to_csv(os.path.join(OUT, 'genes.csv'), index=False)
