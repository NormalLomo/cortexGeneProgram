...
def jorstad_paths_from_source_list(source_list: Path, expected_assets: int=EXPECTED_JORSTAD_ASSETS) -> list[Path]:
    table = pd.read_csv(source_list, sep='\t')
    ...
    for row in table.sort_values('dataset_id').itertuples(index=False):
        path = source_list.parent / row.file_name
        if not path.is_file():
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--jorstad-source-list', type=Path, required=True)
    ...
    parser.add_argument('--raw-layer', default='counts')
...
def prepare_source(path: Path, source_name: str, layer: str, gene_id_column: str, min_counts: int, expected_nuclei: int | None) -> tuple[sparse.csr_matrix, pd.DataFrame, pd.Index]:
...
def combine_sources(paths: list[Path], source_name: str, layer: str, gene_id_column: str, min_counts: int, expected_nuclei: int) -> tuple[sparse.csr_matrix, pd.DataFrame, pd.Index]:
    prepared = [prepare_source(path, f'{source_name}:{path.stem}', layer, gene_id_column, min_counts, None) for path in paths]
    ...
    if common_genes.empty:
        raise ValueError()
    matrix = sparse.vstack([reindex_columns(matrix, genes, common_genes) for (matrix, _, genes) in prepared], format='csr')
...
def main() -> None:
    ...
    jorstad_paths = jorstad_paths_from_source_list(args.jorstad_source_list)
    (j_matrix, j_obs, j_genes) = combine_sources(jorstad_paths, 'Jorstad', args.raw_layer, args.gene_id_column, args.min_counts_per_nucleus, args.expected_jorstad_nuclei)
    (w_matrix, w_obs, w_genes) = prepare_source(args.wei_h5ad, 'Wei', args.raw_layer, args.gene_id_column, args.min_counts_per_nucleus, args.expected_wei_nuclei)
    common_genes = j_genes.intersection(w_genes, sort=False)
    common_genes = common_genes[~common_genes.isin(blacklist)]
    if common_genes.empty:
        raise ValueError()
    matrix = sparse.vstack([reindex_columns(j_matrix, j_genes, common_genes), reindex_columns(w_matrix, w_genes, common_genes)], format='csr').astype(np.float32)
    ...
    result.uns['source_sha256'] = {'Jorstad': {path.name: sha256(path) for path in jorstad_paths}, 'Wei': sha256(args.wei_h5ad)}
